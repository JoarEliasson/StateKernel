...
- `StateKernel.ControlApi`: ASP.NET Core control plane for bounded runtime and run lifecycle orchestration.
- `StateKernel.RuntimeHost`: dedicated runtime process boundary for runtime lifecycle and run/execution orchestration concerns.
- `StateKernel.Simulation`: deterministic, runtime-agnostic execution core.
- `StateKernel.Runtime.Abstractions`: contracts for runtime lifecycle, simulation-to-runtime selection, runtime composition, endpoint/security profile selection, signal projection, and pushed value publication.
- `StateKernel.Runtime.UaNet`: first .NET OPC UA adapter path for read-only signal exposure and bounded endpoint/security profile enforcement.
